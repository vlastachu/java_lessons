...
Честно сказать я не думал, что в этом курсе буду обращать ваше внимание на паттерны проектирования, но похоже, что без них никак и в этой лекции мы изучим пару. Для начала: что такое паттерны проектирования и зачем они нужны?

Паттерны проектирования обычно связаны с ООП (хотя это не является необходимым условием) - просто так сложилось  со временем. 

Зачем они нужны новичкам? вы можете открыть для себя разные композиции, которые вы можете выстраивать из объектов. Ведь изначально ООП представляют студентам, как особенность языка (как я делал ранее). В других источниках ООП представляют как возможность симулировать реальный мир. Объекты там связаны с материальными объектами. Бывают задачи симуляции какой либо системы (и возможно материальной природной системы) - там это уместно. Вроде таким образом появился первый ООП язык Simula (от симуляция). Но это не самая распространенная задача. И в наше время не самое частая область для ООП. Более реальная задача для ООП - представить данные в коде в виде единой сущности к которой у вас имеется необходимый минимум допустимых операций. Паттерны здесь дают вам примеры того как это обычно делают. Хотя многие паттерны (например command, observable) - направлены на то чтобы добавить гибкости в ход выполнения программы (как бы добавляя control-flow конструкции) и не имеют отношения к данным - об этом тоже полезно будет узнать. И ещё один плюс - вас об этом могут спросить на собеседовании в роли junior'а.

Чем это плохо для новичков? Возможно нагружает информацией которая вам не пригодится в ближайшее время. Загоняет вас в рамки, заставляя писать по шаблону (аргумент вроде того, что книги нужны только для тех кто не умеет в познание). Ну и вас об этом будут спрашивать на собеседовании. Получается какой то маркетинговый bullshit. Напрямую вы строить абстрации из паттернов не будете - а вот спросят обязательно. И так программисты из поколение в поколение твердят, что паттерны нужны и что без них никак. Особенно плохо утверждать про паттерны банды четырех (хоть это и классика, но большую часть из них вы наверное не увидете в коде, а некоторые изменились со временем (Observable)). Такие вещи (когда не разобравшись рекомендуют всем подряд) называют культом Карго. Загуглите когда будет свободное время. На английской википедии кажется есть статья про культ Карго в программировании. 

##Паттерн адаптер
Об адапторе я уже говорил ранее. Адаптер - класс использующий реализацию другого класса за счёт агрегирования (включения в класс, как поле класса), заместо наследования. Объект в своих методах пользуется методами содержимого объекта. Это вполне простое явление - этот паттерн существует не для того чтобы показать хороший пример проектирования, а чтобы проименовать агрегирующий класс.

Решает задачу расширения функционала объекта в рантайме. То есть через наследование вы расширяете функционал статически - после компилирования вы уже особо не измените работу методов класса, а в адаптере в свою очередь можно поменять адаптируемый класс.

##Паттерн декоратор

Частный случай адаптера, принимает адаптируемый класс в конструкторе.

Допустим имеется некий корневой класс, либо интерфейс. В котором есть набор методов необходимых для работы с этим классом и хотя бы один конструктор принимающий объект базового класса.

Классический пример - GUI. Допустим есть базовый абстрактный класс View у которого есть методы `void draw()` (и ещё что-нибудь можно придумать например действия при клике мыши и клавиатуры). Мы создаём TextView и как бы декорируем его классами `Scrollable` (добавляем скроллбары), Expanndable (делаем расширяемым текстовое поле):
```java
View text = new Expandable(new Scrollable(new TextView(400, 300)));
...
text.draw();
```
Когда вызывается draw, `Expandable` рисует уголок за который можно тянуть и уровнем ниже рисует `inner.draw();` там ниже `Scrollable` который рисует скроллбары и ниже `TextView`.

То есть в паттерне декоратор в иерархии происходящей от одногой абстрактного элемента наследуются классы, которые производят конечные действия и там же наследуются классы которые оборачивают эти действия и соответственно могут оборачивать друг друга.

Ниже вы встретите декоратор в классах `FilterInputStream`, `FilterOutputStream`.

##Стратегия

Ещё до кучи препишу паттерн стратегию. Она относительно похожа на декоратор и тоже частный случай адаптера. Здесь также композиция осуществляется через конструктор, но всё немного наоборот.

В случае декоратора - декораторы оборачивают основной класс. В случае стратегии основной класс (хотя это может быть не класс, а просто метод), принимает класс с определенным интерфейсом (при этом они не обязаны состоять в общей иерархии), в котором есть методы для реализации конкретного алгоритма. 

Примерно такое можно встретить в `java.util.Collections.sort(Collection<E>, Comparator<E>)`. Допустим по умолчанию строки сортируют в лексикографическом порядке:

```java
		List<String> list = Arrays.asList("2","23", "3", "1", "123");
		Collections.sort(list);
		System.out.println(list);
		// => [1, 123, 2, 23, 3]
```

Допустим нам хочется отсортировать по длине, а затем по символам. Для этого реализуем интерфейс `Comparator<String>` в котором реализуем алгоритм сравнения.
...
В библиотекке java много классов для ввода\вывода. Это связано с тем что есть деление на ввод и вывод. Есть деление на работу с байтами и немного позже первой версии появились классы для работы с символами юникода. Есть классы чтобы брать данные из файлов или из каналов или из сети (хотя это не относится к сети). Есть классы для представления данных в виде строк или массивов или примитивов. Есть классы для специфичной обработки данных. Начиная с java 1.4 кроме пакета io добавился nio (новый io), впрочем io всё ещё актуален.
