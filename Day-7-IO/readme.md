...
Возьмём к примеру задание к 3му дню (например работа Елисеева Дмитрия), после компиляции имеем файлы Main.class и Shape.class (и ещё пара сгенированна). Чтобы запустить их заходим в папку, куда сгенерировали *.class (обычно это out) и в ней вызываем
```
>java -cp "C:\java\jogamp-all-platforms\jar\jogl-all.jar;
C:\java\jogamp-all-platforms\jar\gluegen-rt.jar;." com.company.Main 9
```
(перенос строки я добавил для красоты, а так ещё можно все эти пути в CLASSPATH занести) Получаем:

![снежинка Коха](https://leto26h.storage.yandex.net/rdisk/1b6b5dcd18e24c7e1137266e61f9c529/mpfs/YYrT0mlQEhz0ZnaLCgLrJlEeHLkX1BQ17CSKv6fTrbmVwDcGdHaHCzIheIhDOdxq-n_VkTFr3dFabwqhTO6nzA==?uid=0&filename=2014-10-30%2012-17-48%20%D0%A1%D0%BA%D1%80%D0%B8%D0%BD%D1%88%D0%BE%D1%82%20%D1%8D%D0%BA%D1%80%D0%B0%D0%BD%D0%B0.png&disposition=inline&hash=&limit=0&content_type=image%2Fpng&rtoken=d0120ff7fbec733e71c696778f23eb9b&rtimestamp=5451f4fe&force_default=no)

Согласитесь - для пользователя это немного запарный путь запуска программы. В серьёзных проектах классов будет побольше и иерархия будет тоже пошире. Можно будет пользователю составить скриптик, запускающий нужный класс, а можно сделать один jar-ник. 

Для этого в стандартной поставке jdk предоставляется утилита под названием jar. Она вызывается с флагами, далее имя целевого файла и имя исходных фалов с байт кодом. Например:

```
>jar cf coch.jar .\com\company\*.class
>java -jar coch.jar 9

```

Примерно так. Но это не работает, надеюсь в ближайшее время разберусь, что не так...

jar зачастую используется в java EE вы скидываете на сервер jar его подхватывают фреймворки и начинают с ним работать. Многие библиотеки запиханы в jar (вы уже с этим сталкивались). Кроме того jar можно обернуть в обычный исполняемый файл http://launch4j.sourceforge.net/ правда JRE ползователю все равно понадобится. Можно завернуть и вместе с JRE но это жесть.
...
#Сериализация
Перед вами может возникнуть задача сохранения текущего состояния программы. Например чтобы после закрытия программы восстановить его, либо передать по сети.

Частично эта задача решается через DataOutputStream. Ведь все данные в итоге - композиция из примитивов. Но кроме этого есть ещё ObjectOutputStream (и соответственно input). Из названия уже понятно, что штука достаточно хитрая и делает почти всю работу за нас - не нужно думать о том как бы последовательно представить примитивы, просто сохраним объекты с которыми манипулируем. Единственное условие, что объекты должны реализовать интерфейс `Serializable`.

Из названия интерфейса кажется, что там есть методы `serialize` и `deserialize`, чтобы вручную переводить объект в текст. Но на самом деле в интерфейсе нет ничего. Когда вы пишете `implements Serializable` вы как будто указываете компилятору, что собираетесь в будущем сериализовать объект (не знаю какие меры предпринимает компилятор). 

В ObjectOutputStream мы имеем метод `writeObject(Object)` и с другой стороны имеем метод `Object reedObject()` который кроме прочего может бросить исключение ClassNotFound (вполне логично что в вашей программе может не оказаться такого класса как в файле). Хуже того может оказаться что в вашей программе другая версия класса, нежели та что сохранена. Ну данной информации достаточно чтобы сохранять и читать объекты:
```java
    static class Point3D implements Serializable{
        public int x = 0, y = 0, z = 0;
    }

    public static void main(String[] args) throws IOException, ClassNotFoundException{
        Point3D p = new Point3D(), p2 = null;
        p.y = 2;
        ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream("test"));
        oos.writeObject(p);
        oos.close();
        ObjectInputStream ois = new ObjectInputStream(new FileInputStream("test"));
        p2 = (Point3D)ois.readObject();
        System.out.println(p2.x + " " + p2.y + " " + p2.z);
        // => 0 2 0
    }
```
####transient
Ключевое слово для поля объекта. Ставится также как `final` перед объявлением. Обозначает, что данное поле не будет записывается в файл. Это слово актуально только для классов реализующих `Searializable`. 

####Externalizable
Выше я описал случай, когда вы вручную управляете сохранением объекта. Что если вам действительно будет необходимо сделать вручную? Допустим в объекте приватная информация и её нужно шифровать (для этого случая есть разные шифрующие потоки и запароленные зипы ну да ладно). Для этого есть интерфейс Externalizable в котором необходимо реализовать `void writeExternal(ObjectOutput out)` и `void readExternal(ObjectInput in)`. В этих метода вы сами в задуманном порядке записываете поля и потом в том же порядке их читаете. Ещё одна деталь: при считывании объекта он сначала создаётся конструктором по умолчанию. Так что таковой у вас хотя бы должен быть.

А ещё вы можете для  `Searializable` класса определить следующие методы:
```java
 private void writeObject(ObjectOutputStream stream) throws IOException;
 private void readObject(ObjectInputStream stream) throws IOException, ClassNotFoundException
```
Обратите внимание что они приватные. И вам не придётся делать конструктор по умолчанию. И у вас будет метод `stream.defaultWriteObject` и `stream.defaultReadObject` чтобы проводить обычную сериализацию + ещё какиенибудь  действия.

Ещё пару слов просериализацию: статические поля не сериализуются. Естественно вполне - не записывать же для каждого объекта его статическое поле, потом будет неоднозначность при восстановлении. Вам придётся самому дописать сохранение статического поля.

Ещё я как-то задумался над тем что же будет с final полями которые инициализируются в конструкторе. Ничего страшного не было десериализация прошла как обычно:
...
```java
  static class Point3D implements Serializable{
    ...
    public static void main(String[] args) throws IOException, ClassNotFoundException{
        Point3D p = new Point3D(1, 2, 3), p2 = null;
        ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream("test"));
        oos.writeObject(p);
        oos.close();
        ObjectInputStream ois = new ObjectInputStream(new FileInputStream("test"));
