...
Начиная с java 5 стала доступна форма функций с переменным числом параметров. До этого использовали форму как в примере выше. Чтобы воспользоваться этой формой нам необходимо написать аргумент с его типом и после поставить многоточие. Многоточие говорит о том, что таких аргументов может быть ноль или больше. В коде мы его можем использовать как массив. Нам ничего не стоит переписать пример выше:
...
Как это происходит? Чтобы изобразить картинку нам нужен 4-угольник. Видеокарты манипулируют треугольниками, поэтому мы создаём 2 треугольника. То есть на вход нам нужны 4 координаты и переменная хранящая текстуру. С текстурой у вас никаких проблем не возникнет, а координаты придется просчитывать самим. 

Примерно наш план такой - создать начальные координаты  (чтобы левый нижний угол был в начале координат, а верхний правый был равен ширине, высоте картинки). Затем переместить куда-нибудь в центр часов и повернуть на ныжный нам угол. Нужный угол мы узнаем через `Calendar.getInstance.get(Calendar.SECOND)` - пример для секунд. 

Вообще это можно сделать достаточно в жесткую просчитав каждую точку (хотя с поворотом 4 точек придется немного подумать). Но в OpenGL принято использовать матрицы. Мы передаём двумерный массив для OpenGL - он перемножает матрицу с каждой точкой и графический объект претерпевает некоторые изменения. То есть точка это так называем вектор (мы будет работать в 2D поэтому вектор размера 2), а матрица с которой будем перемножать будет иметь размерность 2х2 - умножаем слева матрицу на вектор (2х2 на 2х1) и получаем вектор 2х1, который и будет новым положением точки.

Насчет некоторых изменений: https://ru.wikipedia.org/wiki/Матрица_перехода - существуют уже готовые матрицы для масштабирования, вращения, перемещения (и ещё есть сдвиг, но он как-то бесполезен). В OpenGL существуют уже готовые функции, для создания и применения этих матриц. Используя их можно вообще не заметить, что вы работаете с матрицами, но знать тем не менее это надо. Когда-нибудь в наткнетесь на такие проблемы как несовпадение размерностей матриц или неправильный порядок перемножения. Поэтому стоит разобраться как оно устроено внутри.

И для этого мы будем вручную создавать матрицы поворота и перемещения матриц и перемножать их. В таком случае все операции будет выполнять процессор, а не видеокарта и это совсем неэффективно, но для около 20 точек - пойдёт. Для начала вам стоит ознакомиться на википедии как выглядят эти матрицы и воссоздать их в соответствующих методах. Тут стоит обратить внимание, что матрицу перемещения размера 2х2 не создать - просто нельзя добиться того чтобы любая точка просто переместилась на определенное расстояние при перемножении с матрицой. Нам придется воспользоваться так называемыми однородными двумерными координатами (термин взял с вики). То есть дополняем наш вектор единицой, а матрицу строим 3х3:
```
1 0 x
0 1 y
0 0 1
```
Перемножаем и берем первые две координаты - они и будут нашими точками. Либо с самого начала используем векторы длиной 3 и у каждого единица будет последней компонентой. А все матрицы (то есть ещё матрица перемещения) размера 3х3 (посмотрите в википедии как выглядит матрица вращения в первом и втором случае).

Когда все методы с матрицами проделаны, нам остаётся их вызвать в правильном порядке:
1. перемещаем часовую стрелку так, чтобы её центр совпадал с началом координат.
2. вращаем её
3. перемещаем в центр часов

Больше теории вы получите на уроках линейной алгебры в курсе матанализа в первом или втором семестре. Я думаю это неплохо что вы будете понимать как эти операции с матрицами могут найти применение на практике.

Кстати изображения стрелок не идеальны (не совсем прозрачная пнг) можете найти получше фон и стрелки, главное константы в начале изменить в соответствии с размерами изображения.

[Необходимый код и изображения](https://github.com/vlastachu/java_lessons/tree/master/src/ru/progexcenter/lesson3/clock)
Инструкция по настройке JOGL см в другом задании.
